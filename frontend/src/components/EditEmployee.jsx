...
const EditEmployee = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    designation: employee?.designation || "",
    gender: employee?.gender || "",
    course: employee?.course ? employee.course : [],
    profileImage: employee?.profileImage || "",
  });
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await fetch("http://localhost:8000/api/course"); // Replace with your API URL
                ...
                courseList.map((course) => (
                  <label key={course._id} className="mx-1">
                    <input
                      ...
                      name="course"
                      value={course.name}
                      checked={formData.course.includes(course.name)}
                      onChange={handleChange}
                    />{" "}
                    {course.name}
                  </label>
